function IMARISDataImport_05072023_V2(celllines_color, celllines, maindir, root, type, tp, pool, progress)
    ...
    function [X, X_labels, T_labels, objectID, cell_list, cellX_labels] = parameter_import(location)
            ...
            for i = 1:length(files) %for every excel file
                ...
                if size(X_temp,2) ~= find(contains(param_labels, 'Volume'))
                    if ~contains(debu_labels, "Dist2Surf")
                        errorfile_list = [errorfile_list; erase(string(modactive_file), '\') append('No Distance to Surface Imported: No Distance to Surface sheet with an proper reference was found')];
                    else
                        errorfile_list = [errorfile_list; erase(string(modactive_file), '\') append('Number of Parameter Mismatch: Imported data is missing or has too many parameters (', string(size(X_temp,2)), ' imported, ' find(contains(param_labels, 'Volume')), ' expected)')];
                    end
                    continue
                end
            ...
            if volume_idx > size(X,2)
                if ~contains(debu_labels, "Dist2Surf")
                    errorfile_list = [errorfile_list; erase(string(modactive_file), '\') append('No Distance to Surface Imported: No Distance to Surface sheet with an proper reference was found')];
                else
                    errorfile_list = [errorfile_list; erase(string(modactive_file), '\') append('Number of Parameter Mismatch: Number of parameters of data (', string(size(X,2)), ') does not match imported data (', string(size(X_temp,2)), ') from file')];
                end
                continue
            end
