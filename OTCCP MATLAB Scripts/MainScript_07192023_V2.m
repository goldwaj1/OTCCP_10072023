...
function CompleteRFScript(celllines_color, celllines, dir, root, script_dir, specialcase, progress, tp, type)
    ...
    if (import == 1)
        IMARISDataImport_05072023_V2(celllines_color, celllines, dir, root, type, tp, pool, progress); cd(script_dir); 
    end

    progressbar('MainScript', root, 'Classification', '', '')
    
    if (rf == 1)
        RandomForest_05082023_V2(celllines_color, celllines, dir, root, progress); cd(script_dir);
    end

    progressbar('MainScript', root, 'Exporting', '', '')
    if (export == 1)
        MATLABDataExport_05072023_V2(dir, root, type, progress); cd(script_dir);
    end
