function IMARISDataImport_10262023_V2(celllines_color, celllines, maindir, root, type, tp, pool, progress)
            ...
            for i = 1:length(files) %for every excel file
                ...
                for k = 1:length(sheets_index) %for every relevant sheet in the excel file
                    ...
                    elseif contains(active_sheet, 'Distance from Origin') || contains(active_sheet, 'Position')
                        ...
                        cellnum_str = regexpi(cellnum_filename, 'cell(.*)?\d*', 'match');
                        cellnum = sscanf(extract(string(cellnum_str), digitsPattern), '%f');
                ...
                cellnum_filename = erase(active_file, match);
                cellnum_str = regexpi(cellnum_filename, 'cell(.*)?\d*', 'match');
                cellnumstr = regexprep(cellnumstr,'[^a-zA-Z0-9]',''); %remove special characters that may not work in CSV files
